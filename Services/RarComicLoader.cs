...
/// <summary>
/// Loads comic pages from RAR and CBR archives (both RAR4 and RAR5 formats) using SharpCompress.
/// </summary>
public sealed class RarComicLoader : IComicLoader
{
    ...
    public Task<ComicBook> LoadAsync(string path, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            archive = RarArchive.OpenArchive(fileStream, options);
        }
        catch (CryptographicException)
        {
            fileStream.Dispose();
            throw new InvalidOperationException($"'{Path.GetFileName(path)}' is password-protected. Password-protected RAR/CBR archives are not supported.");
        }
        catch (Exception ex)
        {
            fileStream.Dispose();
            throw new InvalidOperationException($"Failed to open '{Path.GetFileName(path)}' as a valid RAR/CBR archive: {ex.Message}", ex);
        }
        ...
        try
        {
            ...
            {
                archive.Dispose();
                throw new InvalidOperationException($"'{Path.GetFileName(path)}' contains password-protected entries. Password-protected RAR/CBR archives are not supported.");
            }
        ...
        {
            archive.Dispose();
            throw new InvalidOperationException($"Failed to read '{Path.GetFileName(path)}' as a valid RAR/CBR archive: {ex.Message}", ex);
        }
        ...
        var archiveContext = new RarArchiveContext(archive, validEntries);

        var pages = new List<IComicPage>(validEntries.Count);
        for (int i = 0; i < validEntries.Count; i++)
        {
            string fileName = Path.GetFileName(validEntries[i].Key ?? string.Empty);
            pages.Add(new RarComicPage(i, validEntries[i].Key!, fileName, archiveContext));
        }

        var comicBook = new ComicBook(title, path, ComicSourceType.RarArchive, pages);
        return Task.FromResult(comicBook);
    }

    private sealed class RarArchiveContext : IDisposable
    {
        ...
        public RarArchiveContext(IArchive archive, IEnumerable<IArchiveEntry> entries)
        {
            _archive = archive;
        ...
        public byte[] ReadEntryBytes(string entryKey)
        {
            lock (_syncLock)
            {
                if (_disposed) throw new ObjectDisposedException(nameof(RarArchiveContext));

                if (!_entriesByKey.TryGetValue(entryKey, out var entry))
                {
                    throw new FileNotFoundException($"Entry '{entryKey}' was not found in the RAR archive.");
                }
    ...
    private sealed class RarComicPage : IComicPage
    {
        private readonly string _entryKey;
        private readonly RarArchiveContext _context;

        public int Index { get; }
        public string DisplayName { get; }

        public RarComicPage(int index, string entryKey, string displayName, RarArchiveContext context)
        {
            Index = index;
            _entryKey = entryKey;
